...
// E2B sandbox timeout (30 seconds default)
...
export async function POST(request: NextRequest): Promise<NextResponse<ExecuteResponse>> {
  ...
  try {
    ...
    const { artifact, env, timeout = DEFAULT_TIMEOUT } = body;
    ...
    if (artifact.artifact_type === "frontend" || isFrontendLanguage(artifact.language)) {
      return NextResponse.json(
        { 
          success: false, 
          error: "Frontend code should run in the browser iframe, not E2B. Use artifact_type: 'backend' for server-side code." 
        },
        { status: 400 }
      );
    }
    ...
    // Validate timeout
    const safeTimeout = Math.min(Math.max(1, timeout), MAX_TIMEOUT);

    // Check E2B API key (supports both E2B_API_KEY and E2B_KEY)
    const e2bApiKey = process.env.E2B_API_KEY || process.env.E2B_KEY;
    if (!e2bApiKey) {
      return NextResponse.json(
        { success: false, error: "E2B API key not configured. Set E2B_API_KEY or E2B_KEY in .env" },
        { status: 500 }
      );
    }
    ...
    const result = await executeInSandbox(sandbox, artifact, safeTimeout);
...
export async function GET() {
  return NextResponse.json({
    ...
    request: {
      ...
      timeout: "number (optional) - Execution timeout in seconds (default: 30, max: 120)",
    },
    response: {
      success: "boolean",
      result: {
